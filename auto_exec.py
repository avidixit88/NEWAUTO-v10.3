...
""",
            ...
            if lifecycle.bracket_qty != filled_qty or not (lifecycle.stop_order_id and lifecycle.tp0_order_id):
                ...
                try:
                    ...
                    lifecycle.brackets_last_error = str(e)[:200]
                    lifecycle.notes = (lifecycle.notes or "") + f" | bracket_send_failed:{e}"
                    _record_activity(state, "BRACKETS_FAILED", lifecycle, str(e)[:160])
                    _event_once(
                        cfg,
                        lifecycle,
                        f"BRACKETS_FAILED_{lifecycle.brackets_attempts}",
                        f"[AUTOEXEC] {lifecycle.symbol} BRACKETS FAILED",
                        f"""Time (ET): {now.isoformat()}
Symbol: {lifecycle.symbol}
Engine: {lifecycle.engine}

Entry is filled but bracket placement failed.
OrderId: {lifecycle.entry_order_id}
Filled qty: {filled_qty}
